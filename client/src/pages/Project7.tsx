...
function MultiSelect({ options, selected, onChange, placeholder, labelPrefix = "" }: { options: string[], selected: string[], onChange: (val: string[]) => void, placeholder: string, labelPrefix?: string }) {
  ...
  // Use options directly as provided by UNOS, which are already correctly ordered and include parents
          ...
          <CommandList className="max-h-64 overflow-auto">
             ...
             <CommandGroup>
               {filteredOptions.map((option) => {
                 // For DR51, DR52, DR53, the broad antigens are 51, 52, 53.
                 // The split antigens are 5*01, 5*02, etc., and specific alleles are 5*01:01
                 // A parent is either a number without colon (like 01, 51), or a split without colon (like 5*01)
                 const isParent = !option.includes(':');
                 
                 // Add extra indentation for specific alleles
                 const isSpecificAllele = option.includes(':');
                 
                 // Handle DR51, DR52, DR53 nesting - the broad antigen (e.g. 51) is a top level parent,
                 // the splits (e.g. 5*01) are sub-parents, and the specific alleles (5*01:01) are children
                 const isSubParent = option.includes('*') && !option.includes(':');
                 ...
                 return (
                   <CommandItem
                     ...
                     className={cn(
                       "cursor-pointer",
                       isParent ? "font-bold border-t border-slate-100 mt-1 pt-2 pb-1" : 
                       isSubParent ? "font-medium pl-4 pt-1" : 
                       "pl-8 text-sm text-slate-600"
                     )}
                   >
